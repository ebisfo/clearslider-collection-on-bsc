...
import ClearSlider from '../abis/ClearSlider.json';
import CONFIG from '../config.json';

function App() {
	const [web3, setWeb3] = useState(null)
	const [clearSlider, setClearSlider] = useState(null)
	...
	const loadBlockchainData = async () => {
		...
		if (web3) {
			...
			try {
				const clearSlider = new web3.eth.Contract(ClearSlider.abi, ClearSlider.networks[networkId].address)
				setClearSlider(clearSlider)

				const maxSupply = await clearSlider.methods.maxSupply().call()
				const totalSupply = await clearSlider.methods.totalSupply().call()
				setSupplyAvailable(maxSupply - totalSupply)

				const balanceOf = await clearSlider.methods.balanceOf(account).call()
				setBalanceOf(balanceOf)

				const allowMintingAfter = await clearSlider.methods.allowMintingAfter().call()
				const timeDeployed = await clearSlider.methods.timeDeployed().call()
	...
	const mintNFTHandler = async () => {
		...
		if (clearSlider) {
			...
			await clearSlider.methods.mint(1).send({ from: account, value: 0 })
			...
			<main>
				<Row className="my-3">
					<Col className="text-center">
						<h1 className="text-uppercase">ClearSlider NFTs 1.0</h1>
						{/* <p className="countdown">
							{revealTime !== 0 && <Countdown date={currentTime + (revealTime - currentTime)} />}
						</p> */}
						<p>Mint a ClearSlider. The fastest, most luxurious, most badass 4 door AWD coupe! Free for a limited time! </p>
					</Col>
				</Row>
				<Row className="my-4">
				...
				<Row className="my-3">
					<Col className="flex">
						<a href={openseaURL + account} target="_blank" rel="noreferrer" className="button">How to view and mint</a>
						<a href={`${blockchainExplorerURL}address/${account}`} target="_blank" rel="noreferrer" className="button">My BSC Scan</a>
					</Col>
				</Row>
				<Row className="my-2 text-center">
					{message ? (
						...
						<div>
							{clearSlider &&
								<a href={`${blockchainExplorerURL}address/${clearSlider._address}`}
									target="_blank"
									rel="noreferrer"
									className="contract-link d-block my-3">
									{clearSlider._address}
								</a>
							}
